...
(defun my/setup-fonts ()
  ...
  (when (display-graphic-p)
    ...
    (setq my/han-font
          (cl-loop for font in '("Sarasa Mono SC" "LXGW Neo Xihei" "LXGW WenKai Mono"
                   ...
                   return (progn
                            ...
                            font)))
    ;; 3. 真正“粗体”映射：优先找 原名字-Bold，找不到就把 bold 字号加大
    (when my/han-font
      (let* ((bold-candidates
              (list (concat my/han-font "-Bold")
                    (concat my/han-font " Medium")))
             (real-bold
              (cl-loop for c in bold-candidates
                       when (font-available-p c)
                       return c)))
        (if real-bold
            ;; 找到真正粗体文件 → 直接给 bold 系列 face 用上
            (dolist (face '(bold default-bold mode-line-bold
                                 org-table-bold org-level-1 org-level-2
                                 org-level-3 org-level-4))
              (set-face-font face (font-spec :family real-bold)))
          ;; 找不到 → 把 bold 字号 +10
          (dolist (face '(bold default-bold mode-line-bold))
