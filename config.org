...
;; 保证任何 org 文件在第一次保存时自动插入 CSS 链接
(defun my/ensure-org-css-link ()
  "如果当前 org 文件没有 gongzhitaao 的 CSS 链接，则补上一行。"
  (when (and (eq major-mode 'org-mode)
             (not (save-excursion
                    (goto-char (point-min))
                    (re-search-forward "org\\.css" nil t))))
    (save-excursion
      (goto-char (point-min))
      ;; 定位到最后一个 #+ 关键字行
      (let ((pos (point-min)))
        ...
        (goto-char pos)
        (forward-line 1)
        (insert "#+HTML_HEAD: <link rel=\"stylesheet\" type=\"text/css\" href=\"https://gongzhitaao.org/orgcss/org.css\"/>\n")))
    ;; 保存一次，避免用户下次再手动保存才落盘
    (save-buffer)))
(add-hook 'org-mode-hook
          (lambda ()
            ;; 只在文件从未保存过（新文件）或者本次 Emacs 第一次打开时跑一次
            (unless (bound-and-true-p my/css-link-checked)
              (setq-local my/css-link-checked t)
              (add-hook 'after-save-hook #'my/ensure-org-css-link nil t))))
#+end_src
