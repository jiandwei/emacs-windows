...
(defun my/normalize-path (path)
  "把 Windows 路径转换成 Emacs 惯用的斜杠形式，并确保末尾带斜杠。"
...
(defun my/ensure-directories ()
  "确保 `my/managed-directories' 中的目录存在。
缺失的目录会被一次性创建，之后就不再重复检查。"
  (let ((missing (cl-remove-if #'file-directory-p my/managed-directories)))
    (when missing
      (dolist (dir missing)
        (make-directory dir t)))
    ;; 不论是否创建过目录，一次执行之后就移除自己，避免重复检查。
...
(defun my/detect-fonts-once ()
  ...
  (unless my/font-cache
    (setq my/font-cache
          (list
           :han (cl-loop for f in '("IosevkaTermSS05 Nerd Font Mono + LXGW WenKai Mono Lite Regular" "LXGW WenKai Mono" "Sarasa Mono SC")
...
(use-package eglot
  ...
  (setq-default eglot-workspace-configuration
                '(:python
                  (:basedpyright
                   (:typeCheckingMode "basic"        ;; 降低类型检查强度
                    :useLibraryCodeForTypes nil      ;; 不分析库代码
                    :diagnostics
                    (:enableExperimentalFeatures nil)))
                  :r
                  (:lsp
                   (:diagnostics nil                 ;; 禁用诊断
                    :debug nil
                    :logLevel "error"
                    :rich_documentation t
                    :snippet_support t
                    :max_completions 100             ;; 降低补全数量
                    :lint_cache t))))
...
(use-package pdf-isearch :ensure nil :after pdf-tools)
(use-package pdf-links
  :ensure nil :after pdf-tools
  :bind (:map pdf-view-mode-map
              ("f" . pdf-links-isearch-link)
              ("F" . pdf-links-action-perform)))
(use-package pdf-annot :ensure nil :after pdf-tools)
;; 可选：避免 doc-view 抢占 PDF
(use-package doc-view
  :ensure nil
  :init
  ;; doc-view 遇到 PDF 会调用 doc-view-mode；我们用 pdf-view-mode 接管
