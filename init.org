...
(setq read-process-output-max #x10000)          ; 64 KB
...
;; Make UTF-8 the default coding system for new files, processes, etc.
(prefer-coding-system 'utf-8)
...
(use-package emacs
  ...
  (setq bidi-inhibit-bpa t               ; disable UAX#9 algorithm
        load-prefer-newer t              ; prefer newer elc files
        inhibit-compacting-font-caches t ; keep font caches alive
...
(use-package gptel
  ...
  (setq gptel-default-mode 'org-mode
        gptel-use-curl nil
        gptel-backend (cdr (assoc "Comprehensive" gptel-backends)) ; 默认后端
        gptel-model 'grok-4.1)                                    ; 默认模型
