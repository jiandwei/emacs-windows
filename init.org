...
;; ========== 关键优化 1: 完全避免 package-initialize ==========
(eval-and-compile
  (setq package-enable-at-startup nil)
  
  ;; 直接 require use-package（确保已通过一次性安装）
  ;; 如果未安装，首次运行：M-x package-refresh-contents → M-x package-install RET use-package
  (unless (require 'use-package nil t)
    (warn "use-package 未安装！请手动安装：M-x package-refresh-contents"))
  
  (setq use-package-always-ensure nil
        use-package-expand-minimally t
        use-package-compute-statistics nil))  ;; 关闭统计以减少开销
...
#+begin_src emacs-lisp
    ...
    (unless my/font-cache
      (setq my/font-cache
            (list
             ...
             :symbol (cl-loop for f in '("Symbols Nerd Font Mono" "Apple Symbols"
  ...
  (defun my/apply-fonts-fast (&optional frame)
    "使用缓存快速应用字体配置"
  ...
  (add-hook 'after-init-hook #'my/apply-fonts-fast)
  ;; 新建 frame 时也应用（daemon 模式需要）
  (add-hook 'after-make-frame-functions #'my/apply-fonts-fast)
  ;; Circadian 切换主题后重新应用
  (with-eval-after-load 'circadian
    (add-hook 'circadian-after-load-theme-hook
              (lambda (_theme) (my/apply-fonts-fast))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package company
    ...
    (defvar my/company-enabled-buffers nil
      "已启用 company 的 buffer 列表")

    (defun my/company-enable-on-input ()
      "延迟启用 company-mode，直到首次输入"
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    ...
    :init
    ;; 移除无效的 'ty（lsp-enabled-clients 已废弃或不适用于此）
    ;; (add-to-list 'lsp-enabled-clients 'ty)  ;; 删除这行
    ...
    (setq lsp-keymap-prefix "C-c l"
          ...
          lsp-client-packages '(lsp-python-ty)
...
(use-package format-all
  :ensure t
  :defer t
  :hook ((LaTeX-mode python-mode c-mode org-src-mode) . format-all-mode)
  :init
...
(use-package python
  :defer t                       ; 关键：不立即加载
  :mode ("\\.py\\'" . python-mode) ; 仅当打开 .py 时才加载 python.el
  :init
