...
(progn
  ...
  (eval-when-compile
    (require 'use-package))
  ;; Use USTC mirrors (China) for faster downloads.
...
(use-package doom-modeline
  ...
  :custom
  ;; ---------- Icon settings ----------
  (doom-modeline-icon t)                       ; enable all icons
  (doom-modeline-major-mode-icon t)            ; icon for major mode
  (doom-modeline-major-mode-color-icon t)      ; coloured mode icons

  ;; ---------- Height & bar ----------
  (doom-modeline-height 28)                    ; pixel height of the modeline
  (doom-modeline-bar-width 4)                  ; left highlight bar width
  (doom-modeline-window-width-limit 85)        ; hide info when window < 85 cols

  ;; ---------- What to show ----------
  (doom-modeline-buffer-file-name-style 'relative-from-project) ; short path
  (doom-modeline-buffer-encoding nil)          ; hide encoding (UTF-8 assumed)
  (doom-modeline-indent-info nil)              ; hide indentation style
  (doom-modeline-minor-modes nil)              ; hide minor-mode lighters

  ;; ---------- Time & modal state ----------
  (doom-modeline-time-icon t)
  (doom-modeline-time t)                       ; display clock in modeline
  (doom-modeline-modal t)                      ; show modal editing state
  ...
  ;; ---------- Git ----------
  (doom-modeline-vcs-max-length 20)            ; truncate long branch names
  (doom-modeline-check-simple-format t)        ; simpler git status string
  (doom-modeline-persp-name t)                 ; workspace name (persp/perspective)
  (doom-modeline-persp-icon t)

  ;; ---------- Performance ----------
  (doom-modeline-env-version nil)              ; don't query runtime versions
  (doom-modeline-github nil)                   ; no GitHub notifications
  ...
  ;; ---------- Project ----------
  (doom-modeline-project-detection 'auto)      ; auto-detect project root
  ...
  :config
  ;; ---------- Custom faces (Nord palette) ----------
  (custom-set-faces
   ;; emphasise modified buffer
  ...
  ;; ---------- Auto-adapt to Circadian theme changes ----------
  (with-eval-after-load 'circadian
    (defun my/doom-modeline-adapt-theme ()
      "Adapt modeline faces to the active light/dark theme."
...
(use-package vertico
  ...
  :custom
  (vertico-count 15)                ; 15 candidates visible
...
(use-package marginalia
  ...
  :config
  ;; Custom annotator for commands: show minor-mode state + doc string
  (defun my/marginalia-annotate-command (cand)
    "Annotate command CAND with ON/OFF status and doc string."
    (when-let* ((sym (intern-soft cand)))
      (concat
       ;; minor-mode indicator
       (when (memq sym minor-mode-list)
  ...
  ;; Replace built-in command annotator
...
(use-package org
  ...
  ((org-mode . (lambda ()
                 ...
                 (turn-on-org-cdlatex)          ; fast LaTeX math snippets
  ...
  :config
  ;; ----------------------------------------------------------------
  ;; Image handling: save clipboard images to disk
  ;; ----------------------------------------------------------------
  (setq org-yank-image-save-method "file")

  (defun my/org-yank-image-file-name ()
    "Generate figure path under current heading."
    (unless (buffer-file-name)
      (error "Please save the Org file first"))
  ...
  (defun my/org-indent-setup ()
    "Disable show-paren in Org buffers – it slows things down."
...
(use-package org-habit
  :ensure nil  ;; org-habit 是 org 内置模块
  :after org
  :config
  ;; 习惯图表显示列数
  ...
  (defun my/org-schedule-with-repeat-prompt (orig-fun &rest args)
    "在 org-schedule 后询问是否添加重复"
        ...
        (save-excursion
          ...
          (let ((element (org-element-at-point)))
            (when-let* ((scheduled (org-element-property :scheduled element))
                        (timestamp (org-element-property :raw-value scheduled)))
              (org-schedule nil (concat timestamp " +1d"))))))
      result))
  (advice-add 'org-schedule :around #'my/org-schedule-with-repeat-prompt))
...
(use-package apheleia
  ...
  ;; Python - Ruff 已内置,但需要修正 filepath 的引用方式
  ...
  (setf (alist-get 'python-mode apheleia-mode-alist) 'ruff)  ; 也支持普通 python-mode
...
(use-package gptel
  ...
  (setq gptel-backends
        `(("Comprehensive" .
           ,(gptel-make-openai "Comprehensive"
              :host "elysiver.h-e.top"
              :protocol "https"
              :endpoint "/v1/chat/completions"
              :stream t
              :key #'gptel-api-key-from-auth-source
              :models '(claude-4.5-sonnet-real
  ...
  (setq gptel-default-mode 'org-mode
        ...
        gptel-model 'kimi-k2-thinking)                                    ; 默认模型
