...
(use-package doom-modeline
  ...
  (setq doom-modeline-height 35
        ...
        doom-modeline-minor-modes nil))  ;; 隐藏次要模式图标，避免 clutter
;; -------------------------------------------------------------------
...
(use-package emacs
  :custom
  ;; 指定 ~/.emacs.d/etc/tree-sitter/ 为 Tree-sitter 主目录（优先加载）
  (treesit-extra-load-path
   (cons (expand-file-name "etc/tree-sitter" user-emacs-directory)
         treesit-extra-load-path))
  ;; ------------------------------------------------------------------
  ;; Frame geometry
  ;; ------------------------------------------------------------------
...
(use-package org-modern
  ...
  (setq
   ...
   org-modern-replace-stars '("❖"      ; 黑菱 - 一卷（典籍）
                              "◈"      ; 靶菱 - 二卷
                              "◉")      ; 鱼眼 - 三卷
   org-modern-list '((?+ . "✿")      ; 盛开花头（重要事项）
                     (?- . "❀")      ; 侧影花朵（普通条目）
                     (?* . "✾"))
   ;; 复选框 - 统一方形
   org-modern-checkbox
  ...
  (set-face-attribute 'org-modern-symbol nil
                      :inherit 'default     ;; 继承当前系统字体，不强制指定 family
                      :height 150           ;; 固定 12pt（或根据你的默认字体调整）
                      :weight 'normal)      ;; 确保不加粗影响高度
  ;; 表格 - 优化垂直线
  ...
  (setq org-html-head-extra
        (concat
         ...
         (format "<link rel=\"stylesheet\" type=\"text/css\" href=\"%s\"/>\n"
                 (expand-file-name "assets/org.css" user-emacs-directory))
         ;; 2. Mermaid.js CDN（仅加载库，不初始化）
         "<script src=\"https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js\"></script>\n"
         ;; 3. 本地 org.js（包含初始化、交互功能）
         (format "<script src=\"%s\" defer></script>"
                 (expand-file-name "assets/org.js" user-emacs-directory)))))
...
(use-package markdown-mode
  ...
  ;; 核心：导出时注入外部资源（类似 org-html-head-extra）
  (markdown-content-type "application/xhtml+xml")
  (markdown-xhtml-header-content
   (concat
    ;; 1. 本地 CSS 主题
    (format "<link rel=\"stylesheet\" type=\"text/css\" href=\"%s\"/>\n"
            (expand-file-name "assets/markdown.css" user-emacs-directory))
    ;; 2. Mermaid.js 图表支持
    "<script src=\"https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js\"></script>\n"
    ;; 3. 本地交互脚本（高亮、折叠、暗色切换等）
    (format "<script src=\"%s\" defer></script>"
            (expand-file-name "assets/markdown.js" user-emacs-directory))))
  :custom-face
...
(use-package uv-mode
  ...
  :config
  (setq uv-mode-auto-activate-virtualenv t))

;; Windows 专用：获取 uv 虚拟环境的 Python 路径
(defun my/org-babel-python-use-uv-venv-windows ()
  "Windows 下使用 uv 项目的虚拟环境"
  (let* ((project-root (or (vc-root-dir) default-directory))
         (venv-path (expand-file-name ".venv" project-root))
         (python-exe (expand-file-name "Scripts/python.exe" venv-path)))
    (when (and (file-directory-p venv-path)
               (file-exists-p python-exe))
      (setq-local org-babel-python-command python-exe)
      (message "Org-mode 使用 uv 虚拟环境: %s" python-exe))))

;; 多方式触发
(add-hook 'org-mode-hook #'my/org-babel-python-use-uv-venv-windows)
(add-hook 'org-babel-before-execute-hook #'my/org-babel-python-use-uv-venv-windows)
...
(use-package python
  ...
  (setq major-mode-remap-alist
        '((python-mode . python-ts-mode)))  ; 切换到 tree-sitter 版
...
(use-package prog-mode
  ;; 为所有编程模式绑定钩子：进入 prog-mode 时执行以下配置
  :hook
  (prog-mode . my/prog-mode-config)
  :config
  ;; 定义自定义配置函数，集中管理所有编程模式的设置
  (defun my/prog-mode-config ()
    ...
    (setq-local truncate-lines nil)          ; 关闭硬截断（避免行被截断）
    (setq-local fill-column 80)              ; 填充列指示器的位置（80列）
    (setq-local visual-fill-column-width 80) ; 视觉换行的宽度
    (setq-local visual-fill-column-center-text nil))) ; 不居中文本))
...
(use-package csv-mode
  ...
  ((csv-mode . (lambda ()
                 ...
                 (csv-align-mode 1)
                 ;; 设置列宽限制，防止超宽 CSV 卡死
                 (setq-local csv-align-max-width 40)
                 ;; 显示行号方便查看
                 (display-line-numbers-mode 1)
                 ;; 高亮当前行
                 (hl-line-mode 1)))))
